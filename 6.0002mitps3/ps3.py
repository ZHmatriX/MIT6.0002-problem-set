...
class RectangularRoom(object):
    ...
    def __init__(self, width, height, dirt_amount):
        ...
        self.floors = {(x, y): dirt_amount for x in range(self.width) for y in range(self.height)}
    ...
    def clean_tile_at_position(self, pos, capacity):
        ...
        """
        #获取坐标和dirt_amount,注意得到的类型是float，需要转换
        x,y = (int(pos.get_x()),int(pos.get_y()))
        dirt_amount = self.floors[(x,y)]
        #给对应位置减去capacity，如果减后dirt_amount<=0则将其置为0
        self.floors[(x,y)] = dirt_amount-capacity if dirt_amount>=capacity else 0
    ...
    def is_tile_cleaned(self, m, n):
        ...
        Note: The tile is considered clean only when the amount of dirt on this
              tile is 0.
        """
        return self.floors[(m,n)]==0

    def get_num_cleaned_tiles(self):
        """
        Returns: an integer; the total number of clean tiles in the room
        """
        return sum([1 for dirt_amount in self.floors.values() if dirt_amount==0])
    ...
    def is_position_in_room(self, pos):
        ...
        """
        #possible bug
        return (math.floor(pos.get_x()),math.floor(pos.get_y())) in self.floors
...
class FurnishedRoom(RectangularRoom):
    ...
    def get_random_position(self):
        ...
        #如果该位置无家具，则合法，return，否则重新随机取一个在室内的位置，直至该位置无家具
...
class FaultyRobot(Robot):
    ...
    def update_position_and_clean(self):
        ...
        #随机生成一个新方向
        next_pos = self.position.get_new_position(self.direction, self.speed)
        #如果新方向合法并且gets_faulty为假，则更新位置，打扫地板
        if (self.room.is_position_valid(next_pos) and not self.gets_faulty()):
            self.set_robot_position(next_pos)
            self.room.clean_tile_at_position(next_pos, self.capacity)
        #否则重新设置一个随机方向
        else:
